...
class LoggingPipe:
    def __init__(self, faces_key, output_path, *args, **kwargs):
        ...
        self.headers = ['timestamp', 'face id', 'yaw', 'pitch', 'roll', 'pose_state', 'pose_stable_state',
                        'head_drop_event', 'distracted_event', 'is_yawning', 'head_drops',
                        'perlook', 'yawn_freq', 'head_drop_freq', 'distraction_score', 'distraction_alert', 'fatigue_score', 
                        'fatigue_alert']
    ...
    def save_to_file(self, id, features):
        ...
        output = "{}\n".format(";".join([f"{f:.3f}" if isinstance(f, float) else f"{f}" for f in features]))
        # print(output)
    ...
    def save(self, data):
        ...
        for id, face in faces.items():
            ...
            features = [
                ...
                face['metrics']['head_drops'],
                face['metrics']['perlook'],
                face['metrics']['yawn_freq'],
                face['metrics']['head_drop_freq'],
                face['distraction_score'],
                face['distraction_alert'],
                face['fatigue_score'],
                face['fatigue_alert'],
            ]
