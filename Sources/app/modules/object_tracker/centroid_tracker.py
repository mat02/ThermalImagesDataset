...
class CentroidTracker:
    ...
    def register(self, centroid, bbox_dim, metadata):
        ...
        self.metadata[self.next_object_id] = metadata
    ...
    def update(self, detections):
        ...
        if len(detections) == 0:
            ...
            return self.objects, self.bbox_dims, self.metadata, self.disappeared

        # Initialize an array of input centroids and input bounding box dimensions for the current frame
        input_centroids = np.zeros((len(detections), 2), dtype="int")
        input_bbox_dims = np.zeros((len(detections), 2), dtype="int")

        # Loop over the bounding box rectangles
        for i, detection in enumerate(detections):
            (start_x, start_y, end_x, end_y) = detection["xyxy"]
        ...
        if len(self.objects) == 0:
            for i in range(0, len(input_centroids)):
                self.register(input_centroids[i], input_bbox_dims[i], detections[i])
        ...
        # existing object centroids
            ...
            for (row, col) in zip(rows, cols):
                ...
                self.metadata[object_id] = detections[col]
            ...
            # we need to register each new input centroid as a trackable object
                ...
                for col in unused_cols:
                    self.register(input_centroids[col], input_bbox_dims[col], detections[col])
